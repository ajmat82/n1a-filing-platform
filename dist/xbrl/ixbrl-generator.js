...
class iXBRLGenerator {
    generateiXBRL(fundData) {
        let facts = 0;
        const tag = (name, value, numeric = false) => {
        ...
        const managerRows = managers
            .map((m) => `<li><strong>${tag('PortfolioManagerName', m.name)}</strong>${m.tenureYears !== undefined ? ` — ${esc(m.tenureYears)} yr tenure` : ''}<br><span>${esc(m.bio || '')}</span></li>`)
            .join('\n');
        const boardRows = board
            .map((b) => `<tr><td>${esc(b.name)}</td><td>${b.independent ? 'Independent' : 'Interested'}</td><td>${esc(b.conflicts || '—')}</td></tr>`)
            .join('\n');
...
<body>
  ...
  <ul>${(info.strategies || []).map((s) => `<li>${esc(s)}</li>`).join('') || '<li>—</li>'}</ul>

  <h2>Principal Risks</h2>
  <ul>${(info.principalRisks || []).map((r) => `<li>${esc(r)}</li>`).join('') || '<li>—</li>'}</ul>
