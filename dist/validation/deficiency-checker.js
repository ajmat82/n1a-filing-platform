...
class DeficiencyChecker {
    check(fundData) {
        ...
        const info = fundData.basicInfo || {};
        // ---- Item 2/9: Basic identification ------------------------------------
        if (!info.fundName || !info.fundName.trim())
            err('E_FUND_NAME', 'fundName', 'Fund name is required.');
        ...
        const strategies = (info.strategies || []).filter((s) => s && s.trim());
        const risks = (info.principalRisks || []).filter((r) => r && r.trim());
        if (strategies.length === 0)
            err('E_NO_STRATEGY', 'strategies', 'At least one principal investment strategy is required (Item 4/9).');
        if (risks.length === 0)
            err('E_NO_RISK', 'principalRisks', 'At least one principal risk is required (Item 4).');
        ...
        for (const rule of STRATEGY_RISK_MAP) {
            ...
            if (strategyMentions && !riskDisclosed) {
                warn('W_STRATEGY_RISK_MISMATCH', 'principalRisks', `Strategy references ${rule.label.toLowerCase()} exposure but no matching "${rule.label}" is disclosed in principal risks (strategy/risk correspondence).`);
            }
        }
        // ---- Item 3: Fee table -------------------------------------------------
        ...
        const hasAnyReturn = perf.some((p) => p.annualReturn !== undefined) ||
